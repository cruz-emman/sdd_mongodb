...
const EditAdmin = () => {
  ...
  useEffect(() =>{
    ...
    const getStudentssUser = async () =>{
        try {
            const res = await publicRequest.get(`admins/find/${id}`)
            setInfo({
  ...
  const handleSubmit = async (e) =>{
    e.preventDefault()
    try {
      await publicRequest.put(`admins/${id}`, info)
          ...
          <form onSubmit={handleSubmit} style={{display:'flex', flexDirection:'column', gap:"20px", width: '100%'}}>
            ...
            <FormControl fullWidth>
              ...
              >
                  ...
                  <MenuItem key="ppsa_faculty" value={"ppsa_students"}>PPSA Students</MenuItem>
